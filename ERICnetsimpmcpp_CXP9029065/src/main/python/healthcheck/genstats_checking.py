...
# Description   :  Added support for vCU* and vDU in NRM with specific file size
# Date          :  29/05/2024
...
PRODUCT_DATA_CFG_FILE = '/netsim_users/reference_files/PmConfig_ENM/ProductDataSupportedDeployments/pd_deployment_id_cfg' 
STATS_ONLY_NE_TYPES = [ "CSCF", "EPG-SSR", "EPG-EVR", "M-MGW", "MTAS", "SBG", "VSBG", "SPITFIRE", "TCU03", "TCU04", "MRSV", "MRS", "HSS-FE", "IPWORKS", "MRFV", "UPG", "WCG", "DSC", "VPP", "VRC", "RNNODE", "WMG", "RBS", "STN", "EME", "VTFRADIONODE", "R6274", "R6672", "R6673", "R6675", "R6676", "R6678", "R6371", "R6471-1", "R6471-2","R6273", "VRM", "VRSM", "VSAPC", "VTIF", "PCC", "PCG", "SHARED-CNF", "CCSM", "CCDM", "CCRC", "CCPC", "SC", "CCES", "SMSF", "VNSDS", "CONTROLLER6610", "CIMS" ]
startedNodesFile = "/tmp/showstartednodes.txt"
PLAYBACK_ONLY_NE_TYPES = ["FrontHaul_6020","FrontHaul_6080", "BSP", "vDU", "RDM", "O1" ]
NON_EVENT_FIVEG_NODES = [ "VPP", "VRC", "RNNODE", "5GRADIONODE", "VRM", "VRSM", "VSAPC", "VTIF", "PCC", "PCG", "SHARED-CNF", "CCSM", "CCDM", "CCRC", "CCPC", "SC", "CCES", "SMSF", "vDU", "cIMS", "O1" ]
...
ROUTER_NODE_TYPES = ["R6274", "R6672", "R6673", "R6675", "R6676", "R6678", "R6371", "R6471-1", "R6471-2", "R6273"]
...
FLEX_ROUTER_NODE_TYPES_MD_1 = [ "R6672", "R6675" ]
...
NSS_ONLY_NE_TYPES = ['CCES', 'SMSF', 'VNSDS', 'R6273', 'R6673', 'R6676', 'R6678', 'CONTROLLER6610', "SHARED-CNF", "CIMS"]
...
MD_1_SUPPORTED_NODE_TYPES=["MTAS", "SGSN", "HSS-FE","SCEF", "VCU-UP", "VCU-CP", "SBG-IS", "SCU", "LTE", "ERBS", "R6672", "R6675", "RNC", "RBS", "MSRBS-V2", "PRBS", "BSC"]
NSS_AND_MD_1_ONLY_SUPPORTED_NODE_TYPES=["SCEF"]
PD_SUPPORTED_DEFAULT_NSS_NODES = ["LTE31", "NR18"]
MD_REC_1MIN_SIMS_ONLY = ['SGSN']
...
def check_stats_files(sims, nr_list, lte_uetrace='', playbacksimlist='', edeStatsCheck=False , dpltype="NSS"):
    is_PD_supported = is_productdata_supported(PRODUCT_DATA_CFG_FILE)
    if os.path.isfile(startedNodesFile):
        ...
        for sim in sims:
            ...
            if sim in open(startedNodesFile).read():
                ...
                pool.apply_async(check_stats_each_node, (sim, nr_list, lte_uetrace, playbacksimlist, edeStatsCheck, dpltype, is_PD_supported,))
        pool.close()
        pool.join()
...
def is_productdata_supported(PRODUCT_DATA_CFG_FILE):
    with open(PRODUCT_DATA_CFG_FILE, 'r') as file:
        for line in file:
            match = re.search(r'GENERATE_PD_FILES="(.*?)"', line)
            if match:
                generate_pd_files_value = match.group(1)
                return generate_pd_files_value == 'YES'
    return False



def check_stats_each_node(simname, nr_list, lte_uetrace='', playbacksimlist='', edeStatsCheck=False, dpltype='NSS', is_PD_supported=False):
    pm_data = "/c/pm_data/"
    pd_dir = ""
    ...
    o1_pm_path = "/fs/PerformanceManagementReportFiles"
    ...
    for sim_info in open(SIM_DATA_FILE):
        ...
        if sim_info[1].strip().endswith(simname):
            if "RNC" in sim_info[1].upper() and sim_info[1].strip().endswith(sim_info[3].strip()):
                ...
                if is_PD_supported or (dpltype == "NSS" and any(simname.endswith(s) for s in PD_SUPPORTED_DEFAULT_NSS_NODES)):
                    if sim_info[13] != "/c/pm_data/":
                        pd_dir = sim_info[13]                        
                break
    if edeStatsCheck:
    ...
    # supported types should pass to SimPMVerifier and unsupported should return a warning
    # in MD_1 checking if node_type exists and is not present in supported list or supported list of node types is present in simname then return a warning
    if deploymenttype == 'MD_1' and ((node_type is not "" and node_type.upper() not in MD_1_SUPPORTED_NODE_TYPES) or (not any(ne_type in simname.upper() for ne_type in MD_1_SUPPORTED_NODE_TYPES) or "TSP" in simname.upper())):
        logging.warn("%s  simulation is not supported in " + deploymenttype + " deployment ", simname)
    elif 'sgsn' in simname.lower():
        ...
        x = GenstatsLteSimPmVerifier(TMPFS, simname, stats_dir, trace_dir, pd_dir, lte_uetrace, node_type)
    elif node_type in CONF_EVENTS_NE:
        x = GenstatsSimPmStatsVerifier(TMPFS, simname, stats_dir, pd_dir)
        x.verify()
        ...
        x = GenstatsWranSimPmVerifier(TMPFS, simname, stats_dir, pd_dir, rnc_stats_dir)
    elif 'sbg-is' in simname.lower() and deploymenttype != "DO":
        ...
        elif any(substring in simname.upper() for substring in NSS_AND_MD_1_ONLY_SUPPORTED_NODE_TYPES):
        ...
        elif 'o1' in simname.lower() and deploymenttype == 'NSS':
            x = GenstatsSimPmStatsVerifier(NETSIM_DBDIR, simname, o1_pm_path)
    ...
    #The below method is for handling GSM sims which contains MSC, BSC nodes or MSC, vBSC nodes
        ...
        if 'vbsc' in simname.lower():
           if deploymenttype == "NSS":
              x = GenstatsGsmSimPmVerifier(NETSIM_DBDIR, simname, msc_ready_path, pd_dir)
           else:
              logging.info("%s  simulation is not supported in NRM deployment", simname)
        else:
            x = GenstatsGsmSimPmVerifier(NETSIM_DBDIR, simname, msc_ready_path, pd_dir)
...
def check_crontab(expected_stats_work_load_list, expected_rec_work_load_list,expected_playback_work_load_list, expected_gpeh_work_load_list=[], expected_rbs_gpeh_work_load_list=[], wran=False, gpeh=False, rbs_gpeh=False, dpltype="NSS"):
    ...
    if wran:
        ...
        if dpltype == "MD_1":
            if not check_flex_nodes(MD_REC_1MIN_SIMS_ONLY, False) and '1' in expected_rec_work_load_list:
                expected_rec_work_load_list = [value for value in expected_rec_work_load_list if value != '1']
...
''' % (program_shortdesc)
    ...
    elif "NRM" in deploymenttype or "MD_1" in deploymenttype:
    ...
    elif (statsworkload is not None and recworkload is not None):
        if deploymenttype == "MD_1":
            check_crontab([x.split(":")[0] for x in statsworkload], [x.split(":")[0] for x in recworkload], [x.split(":")[0] for x in playback_work_load],dpltype=deploymenttype)
        else:
            check_crontab([x.split(":")[0] for x in statsworkload], [
