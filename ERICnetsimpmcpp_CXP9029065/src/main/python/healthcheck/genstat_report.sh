...
################################################################################
# Version no    :  NSS 24.02
# Purpose       :  This script is responisible for initiating health check w.r.t stats and events workload.
# Jira No       :  NSS-46297
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/16701382/
# Description   :  Requirement for new NRM to be designed for Mediation Dimensioning Protection Loop
# Date          :  28/11/2023
...
executeHealthCheck(){
    ...
    GENSTATS_CHECKING_NEE="${BIN_DIR}/genstats_checking.py -l ${SIM_LIST} -ul ${LTE_UETRACE_LIST} ${MSRBS_V1_LTE_UETRACE_LIST} ${MSRBS_V2_LTE_UETRACE_LIST} ${VTF_UETRACE_LIST} -b ${SET_BANDWIDTH_LIMITING} -recwl ${RECORDING_WORKLOAD_LIST}  -statswl ${STATS_WORKLOAD_LIST} -gpehwl ${GPEH_WORKLOAD_LIST} -rbsgpehwl ${GPEH_RBS_WORKLOAD} -gpehmpcells ${GPEH_MP_CONFIG_LIST} -deployment ${TYPE} -edeStatsCheck ${edeStatsCheck} -periodicHC ${periodic_bool} -epoch_time ${SYS_EPOCH_TIME} -current_time ${CURRENT_EPOCH_TIME}"
    ...
    if [[ ! -z ${CONSOLIDATED_SIM_LIST} ]]; then
        if [ ${TYPE} == "NRM1.2" ]; then
            ...
            if [[ ! -z ${PLAYBACK_SIM_LIST} ]];then
                ...
                python ${GENSTATS_CHECKING_NEE} -current_time ${CURRENT_EPOCH_TIME}
            fi
        else
            if [[ ! -z ${PLAYBACK_SIM_LIST} ]];then
