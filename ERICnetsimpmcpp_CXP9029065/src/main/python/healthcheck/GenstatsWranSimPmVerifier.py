...
class GenstatsWranSimPmVerifier(GenstatsSimPmVerifier):
    ...
    def __init__(self, tmpfs_dir, simname, pm_data_dir, pd_dir='', rnc_parent_dir=''):
        '''
        Constructor
        '''
        global max_rnc_ids_supported
        self.pd_dir = pd_dir
        ...
        max_rnc_ids_supported = 21
        depl_type = util.getDeploymentVersionInformation()
        if depl_type == "MD_1":
            max_rnc_ids_supported = 40
    ...
    def verify(self):
        ...
        if not any( 'PRBS' in node and 'RNC' in self.simname for node in self.nodename_list):
            ...
            if int(self.simname.replace('RNC','')) < max_rnc_ids_supported:
            ...
            if self.pd_dir:
                self.report_error(self.simname + " MISSING PD FILES ",self.get_nodes_file_not_generated,self.mixed_mode_nodename_list, self.pd_dir, '*.tar.gz.gpg*', self.rnc_parent_dir)
