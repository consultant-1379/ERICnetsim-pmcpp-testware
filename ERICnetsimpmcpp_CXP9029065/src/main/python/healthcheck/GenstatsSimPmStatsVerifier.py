...
class GenstatsSimPmStatsVerifier(GenstatsSimPmVerifier):
    ...
    def __init__(self, tmpfs_dir, simname, pm_data_dir, pd_dir=""):
        '''
        Constructor
        '''
        self.pd_dir = pd_dir
    ...
    def verify(self):
        ...
        if "SCEF" in self.simname.upper():
           for path_info in self.pm_data_dir.split("|"):
            ...
            # 5GRADIONODE does not represent node type LTE MSRBS-V2
            if self.pd_dir and "5GRADIONODE" not in self.simname.upper():
                self.report_error(self.simname + " MISSING PD FILES ",
                                  super(
                                      GenstatsSimPmStatsVerifier, self).get_nodes_file_not_generated,
                                  self.nodename_list, self.pd_dir, '*.tar.gz.gpg*')
