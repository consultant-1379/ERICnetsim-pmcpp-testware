...
# Jira No       :  NSS-43613
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/17905280/
# Description   :  Removed mount verifier code to continue investigation on AUTO_DETECT_NRM issue
# Date          :  22/04/2024
...
class GenstatsSimPmVerifier(object):
    ...
    def get_nodes_file_not_generated(self, nodename_list=[], data_dir='', reg='*', rnc_parent_dir=''):
        result = []
        simname_upper = self.simname.upper().strip()
        for nodename in nodename_list:
            if 'RNC' in simname_upper and self.simname.strip().endswith(nodename.strip()):
                pm_data_path = self.tmpfs_dir + \
                    self.simname + '/' + nodename + rnc_parent_dir
            elif 'BSC' in simname_upper and 'MSRBS' in nodename:
                if data_dir == '/productdata/':
                    pm_data_path = self.tmpfs_dir + \
                        self.simname + '/' + nodename + '/fs' + data_dir
                else:
                    pm_data_path = self.tmpfs_dir + \
                        self.simname + '/' + nodename + '/fs/c/pm_data'
            elif 'BSC' in simname_upper and data_dir == '/productdata/':
                continue
            else:
                pm_data_path = self.tmpfs_dir + \
