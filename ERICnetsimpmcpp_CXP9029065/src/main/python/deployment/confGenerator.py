...
# Jira No       :  NSS-47504
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/17426859/
# Description   :  Added bulkPM support for the 669x sims
# Date          :  04/04/2024
    ...
    if arg_map.has_key('sim_name') and arg_map.has_key('node_name'):
        if arg_map.has_key('nodeType') and 'Mini-Link' in arg_map.get('nodeType'):
            ...
            if arg_map.has_key('rcID'):
                ...
                if arg_map.has_key('node_category'):
            ...
            elif arg_map.get('gran_period') in ['4','8']:
                arg_map["fileToBeAssembled"] = '669x'
            else:
...
def getAttributeValue(str_):
    ...
    if MINILINK_MO_TYPE == 'ETHERNET':
        if 'filename' == str_:
            ...
            if 'xfXmlPmFileGranularityPeriod' in arg_map:
                attr = 'xfXmlPmFileGranularityPeriod'
            else:
...
def setUserLableAndLocalDn(file_name):
    ...
    if len(node_info) == 1:
        ...
        if node_info[1].endswith(".xml.gz"):
            node_info[1] = node_info[1].replace(".xml.gz", "")
            arg_map["userLabel"] = node_info[0].split('=')[1] + '_' + node_info[1]
            arg_map["localDn"] = node_info[0] + '_' + node_info[1]
        else:
