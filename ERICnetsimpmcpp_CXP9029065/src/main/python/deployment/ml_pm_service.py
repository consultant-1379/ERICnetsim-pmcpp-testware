...
####################################################
# Version no    :  NSS 24.08
# Purpose       :  Creates ML outdoor stats files
# Jira No       :  NSS-48417
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/17883780/
# Description   :  Added interval list bulkPM support for ML6352 23.Q1 versions
# Date          :  22/04/2024
...
timestamp_mapping, fileInformationMap, processed_node_map, timestamp_mapping_for_ml6352_a_15 = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
unique_timestamps_list_of_c_sample_for_new_reqt = []
timestamps_list_for_ml6352_c_file_one_day_rop_for_new_reqt = []
timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt = []
...
file_writer_order = {'A' : ['A_HEADER', 'A_SAMPLE', 'A_FOOTER'], 'C' : [ 'C_HEADER', 'C_SAMPLE', 'C_FOOTER'],
                     ...
                     'ML6352_NEW_A_15' : ['A_HEADER_15', 'A_NEW_SAMPLE_15', 'A_FOOTER_15'], 'ML6352_NEW_A_24' : ['A_HEADER_24', 'A_NEW_SAMPLE_24', 'A_FOOTER_24']}
ml_outdoor_special_map = {'ML6352_NEW_REQT' : {'A_SAMPLE' : 'Ml6352_New_Request_MiniLink_Outdoor_A_Sample_File.xml', \
                          ...
                          'ML6352_NEW_REQT_23_Q1' : {'A_SAMPLE_15' : 'Ml6352_New_Request_MiniLink_Outdoor_A_15_Sample_File.xml', \
                                                     'A_SAMPLE_24' : 'Ml6352_New_Request_MiniLink_Outdoor_A_24_Sample_File.xml'}}
...
def generatePmFile(file_type, file_name, location, localdn, userlabel, sf_version, new_reqt, interval = None, start = None):
    ...
    elif new_reqt and interval != None:
        start_date = timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt[96-int(interval)]
        end_date = timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt[96-int(interval)+1]
        start_date_in_ISO_format = timestamp_mapping_for_ml6352_a_15[start_date]
        end_date_in_ISO_format = timestamp_mapping_for_ml6352_a_15[end_date]
        sample_time_stamp = fileInformationMap['A_NEW_SAMPLE_15'][1].split('"')[3]
        fileInformationMap['A_HEADER_15'][5] = fileInformationMap['A_HEADER_15'][5].replace(fileInformationMap['A_HEADER_15'][5].split('"')[1], start_date_in_ISO_format)
        fileInformationMap['A_FOOTER_15'][2] = fileInformationMap['A_FOOTER_15'][2].replace(fileInformationMap['A_FOOTER_15'][2].split('"')[1], end_date_in_ISO_format)
               ...
               for elem_dict in list_of_dictionaries_of_c_sample_for_new_reqt:
                   for key in elem_dict:
                       if key in required_rop_list_from_request:
                          ...
                          pm_out.write(line)
            if order == 'A_NEW_SAMPLE_15':
                for line in range(len(fileInformationMap[order])):
                    if 'endTime=' in fileInformationMap[order][line]:
                        fileInformationMap[order][line] = fileInformationMap[order][line].replace(sample_time_stamp, end_date_in_ISO_format)
...
def process_request(req):
    ...
    if 'interval_list' in request_information:
        for ROP_request in request_information['interval_list'].split(','):
            if '-' not in ROP_request:
                interval_list.append(ROP_request)
            else:
                interval_list+=[ ROP_req for ROP_req in range(int(ROP_request.split('-')[0]),int(ROP_request.split('-')[1])+1)]
    ip_address = None
       ...
       if 'interval_list' in request_information:
          if request_information['interval_min'] != '1440':
              for interval in interval_list:
                  time_stamp = timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt[96-int(interval)+1]
                  a_rop_file_name = ml6352_a_rop_file_format_15.replace('<FILE_TYPE>', 'A').replace('<TIME_STAMP>', time_stamp).replace('<USER_LABEL>', user_label)
                  if not os.path.isfile(dest_path + a_rop_file_name):
                      generatePmFile('A_15', a_rop_file_name, dest_path, localDn, user_label, request_information['softwareVersionRelease'], True, interval=interval )
          else:
              a_rop_file_name = ml6352_a_rop_file_format_24.replace('<FILE_TYPE>', 'A').replace('<USER_LABEL>', user_label)
              generatePmFile('A_24', a_rop_file_name, dest_path, localDn, user_label, request_information['softwareVersionRelease'], True)
       elif request_information['interval_min'] != '1440':
          ...
          generatePmFile('C', c_rop_file_name, dest_path, localDn, user_label, request_information['softwareVersionRelease'], True, start=request_information['start_interval'])
       else:
...
def generateRopFileNameFormat():
    ...
    global ml6352_a_rop_file_format_15
    global ml6352_a_rop_file_format_24
    ...
    ml6352_a_rop_file_format_15 = '<FILE_TYPE>' + '<TIME_STAMP>' + '_ManagedElement=' + '<USER_LABEL>' + '.xml'

    ml6352_a_rop_file_format_24 = '<FILE_TYPE>' + getTimeInNormalFormat(current_time,24) + '_ManagedElement=' + '<USER_LABEL>' + '.xml'
...
def getTimeInNormalFormat(time, rop):
    if rop == 15:
        t2=time+fifteen_min_in_sec
        return strftime('%Y%m%d.%H%M', localtime(time)) + util.generateOffSetValue(time)[0].replace(':','') + strftime('-%H%M', localtime(t2)) + util.generateOffSetValue(t2)[0].replace(':','')
    else:
        #t1=((int(time - one_day_in_sec) / one_day_in_sec) * one_day_in_sec)
        #t2=t1+one_day_in_sec
        t1 = localMidnight
        t2 = localMidnight + one_day_in_sec
        return strftime('%Y%m%d.0000', gmtime(t1)) + util.generateOffSetValue(t1)[0].replace(':','') + strftime('-%Y%m%d.0000',gmtime(t2)) + util.generateOffSetValue(t2)[0].replace(':','')
...
def consumeDataForMl6352NewRequirement23Q1():
    ml6352_new_a_15_sample = os.path.join(minilink_template_location, ml_outdoor_special_map['ML6352_NEW_REQT_23_Q1']['A_SAMPLE_15'])
    with open(ml6352_new_a_15_sample, 'r') as s_fin:
        for line in s_fin:
            fileInformationMap['A_NEW_SAMPLE_15'].append(line)
    fileInformationMap['A_HEADER_15'] = copy.copy(fileInformationMap['C_HEADER'])
    fileInformationMap['A_HEADER_15'][4] = fileInformationMap['A_HEADER_15'][4][:16] + 'elementType="MINI-LINK 6352 80/21H" ' + fileInformationMap['A_HEADER_15'][4][16:]
    fileInformationMap['A_FOOTER_15'] = copy.copy(fileInformationMap['C_FOOTER'])
    ml6352_new_a_24_sample = os.path.join(minilink_template_location, ml_outdoor_special_map['ML6352_NEW_REQT_23_Q1']['A_SAMPLE_24'])
    start_date_in_ISO_format = getTimeInIsoFormat(localMidnight, type=True)
    end_date_in_ISO_format = getTimeInIsoFormat(localMidnight + one_day_in_sec, type=True)
    with open(ml6352_new_a_24_sample, 'r') as s_fin:
        for line in s_fin:
            if 'endTime' in line:
                line = line.replace(str(line.split('"')[3]), end_date_in_ISO_format)
            fileInformationMap['A_NEW_SAMPLE_24'].append(line)
    fileInformationMap['A_HEADER_24'] = copy.copy(fileInformationMap['A_HEADER'])
    fileInformationMap['A_HEADER_24'][4] = fileInformationMap['A_HEADER_24'][4][:16] + 'elementType="MINI-LINK 6352/2 80/21L" ' + fileInformationMap['A_HEADER_24'][4][16:]
    fileInformationMap['A_HEADER_24'][5] = fileInformationMap['A_HEADER_24'][5].replace(fileInformationMap['A_HEADER_24'][5].split('"')[1], start_date_in_ISO_format)
    fileInformationMap['A_FOOTER_24'] = copy.copy(fileInformationMap['A_FOOTER'])
    fileInformationMap['A_FOOTER_24'][2] = fileInformationMap['A_FOOTER_24'][2].replace(fileInformationMap['A_FOOTER_24'][2].split('"')[1], end_date_in_ISO_format)
...
def generateTimeStampListForOneDayRopCFileMl6352():
    ...
    global timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt
    global timestamp_mapping_for_ml6352_a_15
    start_end_time_for_c_sample = current_time - one_day_in_sec + fifteen_min_in_sec
    start_end_time_for_a_15_sample = current_time - one_day_in_sec
    for i in range(0,96):
        ...
        timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt.append(getTimeInNormalFormat(start_end_time_for_a_15_sample + (i * fifteen_min_in_sec),15))
    timestamp_mapping_for_ml6352_a_15 = dict(zip(timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt, timestamps_list_for_ml6352_c_file_one_day_rop_for_new_reqt))
    timestamp_mapping_for_ml6352_a_15[getTimeInNormalFormat(start_end_time_for_a_15_sample-fifteen_min_in_sec,15)]=getTimeInIsoFormat(start_end_time_for_c_sample-fifteen_min_in_sec)
    timestamps_list_for_ml6352_a_15_new_file_one_day_rop_for_new_reqt.insert(0,getTimeInNormalFormat(start_end_time_for_a_15_sample-fifteen_min_in_sec,15))
...
def main():
    ...
    if os.path.isdir(precooked_folder):
        ...
        consumeDataForMl6352NewRequirement23Q1()
