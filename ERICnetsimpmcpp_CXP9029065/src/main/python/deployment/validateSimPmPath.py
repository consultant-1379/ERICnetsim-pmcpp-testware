...
def generateNePmPathMap(cfg):
    ...
    with open(sim_data_file, 'r') as fin:
        for line in fin:
            ...
            node_type, stats_dir, trace_dir, pd_dir = lineElements[5], lineElements[9], lineElements[11], lineElements[13]
            ne_pm_path_map[node_type]['STATS'].append(stats_dir)
            ne_pm_path_map[node_type]['TRACE/EVENTS'].append(trace_dir)
            ne_pm_path_map[node_type]['PD'].append(pd_dir)
    if ne_pm_path_map:
