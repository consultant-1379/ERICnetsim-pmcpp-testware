...
###################################################
# Version no    :  NSS 24.01
# Purpose       :  Script compares stats file sizes generated on the server with expected ones stored in cfg file
# Jira No       :  NSS-46392
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/16548744/
# Description   :  Added missing configureation which impacted by NRM6.4 missing changes
# Date          :  30/10/2023
...
PM_STATS_FILESIZE_CFG = '/netsim_users/reference_files/PmConfig_ENM/ExpectedPMStatsFileSizes/Default/pm_stats_filesize_cfg'
PM_STATS_VERIFICATION_PARAM = 'PM_STATS_VERIFICATION_PARAM'
NE_STATS_MAPPINGS = defaultdict(lambda : defaultdict(list))
DUAL_PM_PATH = ["CUDB"]
DIFF_FILE_SIZES_SUPPORTED_NRM = ["NRM6.4"]
...
def updateFileSizeDict():
    ...
    if dep_type in DIFF_FILE_SIZES_SUPPORTED_NRM:
        PM_STATS_FILESIZE_CFG = '/netsim_users/reference_files/PmConfig_ENM/ExpectedPMStatsFileSizes/'+dep_type.strip()+'/pm_stats_filesize_cfg'
    ...
    with open(SIM_DATA_FILE, 'r') as sim_file:
        for line in sim_file:
            ...
            for extension, ne_map in NE_STATS_MAPPINGS.iteritems():
                if line_info[5] == 'GNODEBRADIO' and dep_type in ["NRM6.3", "NRM6.4"]:
...
def main():
    global master_server, servers, dep_type
    if len(sys.argv) > 1:
    ...
    dep_type = util.getDeploymentVersionInformation()
