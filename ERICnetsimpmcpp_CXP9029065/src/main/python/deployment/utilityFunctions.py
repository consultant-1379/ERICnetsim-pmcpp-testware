...
#############################################################################
# Version no    :  NSS 23.16
# Purpose       :  Has various generic utility methods.
# Jira No       :  NSS-39681
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/15442763/
# Description   :  Genstats NR support - NRM6.4: 100K NRM build Sample sims testing
# Date          :  12/09/2023
...
class Utility:
    ...
    events_file_format_mapping = {'FIVEGRADIONODE' : { 'CELLTRACE' : ['A<START_DATE>.<START_TIME>-<END_TIME>_CellTrace_<EventProducer><FILE_ID>.gpb', '.gz']},
                                  'GNODEBRADIO' : { 'CELLTRACE' : ['A<START_DATE>.<START_TIME>-<END_TIME>_CellTrace_<EventProducer><FILE_ID>.gpb', '.gz']}}
                        ...
                        'FIVEGRADIONODE' : {'low' : {'CUCP': 'celltrace_cucp_2304K.bin.gz',
                                       ...
                                       'high' : {'CUCP' : 'celltrace_cucp_768k.bin.gz',
                                                  'CUUP' : 'celltrace_cuup_768k.bin.gz',
                                                   ...
                                                   'default' : 'celltrace_768K.bin.gz'}}},
                     3: {'GNODEBRADIO' : {'low' : {'CUCP' : 'celltrace_cucp_1.6MB.bin.gz',
                                                  'CUUP' : 'celltrace_cuup_804KB.bin.gz',
                                                    'DU' : 'celltrace_du_1.7MB.bin.gz',
                                                     'default' : 'celltrace_1MB.bin.gz'},
                                            'high' : {'CUCP' : 'celltrace_cucp_526KB.bin.gz',
                                                    'CUUP' : 'celltrace_cuup_272KB.bin.gz',
                                                     'DU' : 'celltrace_du_576KB.bin.gz',
                                                     'default' : 'celltrace_333KB.bin.gz'}},
                        'FIVEGRADIONODE' : {'low' : {'CUCP': 'celltrace_cucp_2304K.bin.gz',
                                               'CUUP': 'celltrace_cuup_2304K.bin.gz',
                                               'DU': 'celltrace_du_2304K.bin.gz',
                                       ...
                                       'high' : {'CUCP' : 'celltrace_cucp_768k.bin.gz',
                                                  'CUUP' : 'celltrace_cuup_768k.bin.gz',
                                                   'DU' : 'celltrace_du_768k.bin.gz',
                                                   'default' : 'celltrace_768K.bin.gz'}}},
                     4: {'GNODEBRADIO' : {'low' : {'CUCP' : 'celltrace_cucp_94byte.bin.gz',
                                                  'CUUP' : 'celltrace_cuup_91byte.bin.gz',
                                                    'DU' : 'celltrace_du_91byte.bin.gz',
                                                     'default' : 'celltrace_1MB.bin.gz'},
                                            'high' : {'CUCP' : 'celltrace_cucp_94byte.bin.gz',
                                                    'CUUP' : 'celltrace_cuup_91byte.bin.gz',
                                                     'DU' : 'celltrace_du_91byte.bin.gz',
                                                     'default' : 'celltrace_333KB.bin.gz'}},
                        'FIVEGRADIONODE' : {'low' : {'CUCP': 'celltrace_cucp_94byte.bin.gz',
                                               'CUUP': 'celltrace_cuup_91byte.bin.gz',
                                               'DU': 'celltrace_du_91byte.bin.gz',
                                               'default' : 'celltrace_768K.bin.gz'},
                                       'high' : {'CUCP' : 'celltrace_cucp_94byte.bin.gz',
    ...
    def mountDirectory(self, src_path, dest_path):
        ...
        try:
            dest_path = filter(None, dest_path.split('/'))
    ...
    def createManifestFile(self, manifest_map):
        ...
        try:
            ...
            for directory, fileList in manifest_map.iteritems():
        ...
        with open(self.netsim_cfg, 'r') as f:
            for line in f:
                if line.startswith(searchString):
                    return filter(None, line.split('"')[1].split())
    ...
    def filterNodePathListFromStartedNodeFile(self, input_node_path_list):
        ...
        try:
            ...
            for node_path in input_node_path_list:
                ...
                node_name = os.path.basename(node_path).strip()
                if node_name:
                    for line in startedFile:
                        if node_name in filter(None, line.split()):
    ...
    def getherNodeList(self, pms_path, sim_name):
        """ This method return list of node directory with full path based on pms path and sim name given.
            i.e : /pms_tmpfs/sim_name/node_name
        """
        try:
            if self.checkDirectoryExistance(pms_path + sim_name):
                return [ pms_path + sim_name + '/' + path for path in filter(None, os.listdir(pms_path + sim_name))]
            else:
                self.printStatements('Directory ' + pms_path + sim_name + ' is not present.', 'WARNING')
                return []
        except:
            traceback.print_exc()
    ...
    def checkForNodeStatus(self, inputNode, inputFile):
        ...
        try:
            f = open(inputFile, 'r')
            for line in f:
                if inputNode in filter(None, line.split()):
                    startedNode = True
                    break
        except:
            traceback.print_exc()
    ...
    def fetchNeList(self, inputPath):
        if self.checkDirectoryExistance(inputPath):
            return [ folder_list for folder_list in filter(None, os.listdir(inputPath)) ]
        else:
            return []
