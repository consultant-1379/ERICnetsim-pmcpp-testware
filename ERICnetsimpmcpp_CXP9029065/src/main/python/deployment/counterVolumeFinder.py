...
def findCounterType(inLine):
    ...
    if counter_type == 'SINGLE_VALUE':
        ...
        if lineElements[4] == 'UNCOMPRESSED':
            ...
            throwConsoleLogs(err, 'Invalid Multi-value type Counter found : ' + line)
    else:
        throwConsoleLogs(err, 'Invalid Counter type found : ' + line)
...
def readCounterPropertyFile():
    ...
    with open(counterPropFile, READ_MODE) as inProp:
        for line in inProp:
            ...
            if not counterPropMap.has_key(mo_name):
                counterPropMap[mo_name][counter_name] = counter_type
            else:
                if not counterPropMap[mo_name].has_key(counter_name):
                    counterPropMap[mo_name][counter_name] = counter_type
                else:
...
def returnMoVolume(mo, cntList, cntValList):
    ...
    for index, name in enumerate(cntList):
        if counterPropMap.has_key(mo):
            if counterPropMap[mo].has_key(name):
