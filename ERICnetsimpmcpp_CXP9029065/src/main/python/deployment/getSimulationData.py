...
SPITFIRE_NE_TYPES = ["SPITFIRE", "R6274", "R6672", "R6673", "R6676", "R6678", "R6675", "R6371", "R6471-1", "R6471-2", "R6273"]
HSS_NE_TYPE = "HSS-FE"
...
SUPPORTED_NE_TYPES = [ "CSCF", "EPG-EVR", "EPG-SSR", "MGW", "MTAS", "LTE", "MSRBS-V1", "MSRBS-V2", "PRBS", "ESAPC", "SBG", "VSBG", "SGSN", "SPITFIRE", "TCU", "RBS", "RNC", "RXI", "VBGF", "HSS-FE", "IPWORKS", "C608", "MRF", "UPG", "WCG", "VPP", "VRC", "RNNODE", "DSC", "WMG", "SIU", "EME", "VTFRADIONODE", "5GRADIONODE", "R6274", "R6672", "R6673", "R6676", "R6678", "R6675", "R6371", "R6471-1", "R6471-2", "R6273", "VRM", "VRSM", "VSAPC", "VTIF", "MRS", "GNODEBRADIO", "PCC", "PCG", "CCSM", "CCDM", "CCRC", "CCPC", "SC", "CCES", "VNSDS", "CONTROLLER6610", "SMSF", "SHARED-CNF", "CIMS" ]
...
configFileLocation = "/netsim_users/reference_files/PmConfig_ENM/DefaultStatsAndEventsPmPaths/"
...
depl_type="NSS"
# Genstats treats MSRBS-V2 for 5G Sims as GNODEBRADIO except for LTE-5GNODEBRADIO
PD_NE_TYPES = ["MSRBS-V2", "GNODEBRADIO"]
    ...
    for sim in sim_info_map:
        ...
        for node in node_info:
            ...
            if not trace_dir:
                ...
                continue
            # Get path data for pd files only by adding pd_ to data_dir as it can be the same for different file types but dumpmptree is different
            if node_type in PD_NE_TYPES:
                pd_dir = get_pmdata_mo_attribute_value("pd_"+data_dir, sim, node_name, node_type, mim_ver)  
            else:
                pd_dir = "/c/pm_data/"
            if not pd_dir:
                print "WARN : "+ sim +" do not have pd_dir set. Please check"
                continue
            ...
            if not trace_dir.endswith("/"):
                trace_dir = trace_dir + "/"
            if not pd_dir.endswith("/"):
                pd_dir = pd_dir + "/"

            sim_information = "sim_name: " + sim + "\tnode_name: " + node_name + " \tnode_type: " + node_type + "\tsim_mim_ver: " + mim_ver + "\tstats_dir: " + stats_dir + "\ttrace: " + trace_dir +  "\tpd_dir: " + pd_dir + "\tmim: " + mim_file
...
def  get_pmdata_mo_attribute_value(data_dir, sim_name, node_name, node_type, mim_ver):
    ...
    mo_fdn = ""
    if node_type in CPP_NE_TYPES:
        if "pd_performanceDataPath" in data_dir:
            data_dir = data_dir.replace("pd_", "")
            mo_attribute = data_dir
            mo_fdn = "dumpmotree:moid=\"ComTop:ManagedElement=" + node_name + ",RmeSupport:NodeSupport=1,RmeCcpdService:CcpdService=1\",printattrs;"
        elif "fileLocation" in data_dir:
    ...
    if node_type in MSRBS_V2_NE_TYPES:
        if "pd_fileLocation" in data_dir:
            data_dir = data_dir.replace("pd_", "")
            mo_attribute = data_dir
            mo_fdn = "dumpmotree:moid=\"ComTop:ManagedElement=" + node_name + ",RmeSupport:NodeSupport=1,RmeCcpdService:CcpdService=1\",printattrs;"
        ...
        elif "outputDirectory" in data_dir:
            mo_attribute = data_dir
        ...
        if "pd_fileLocation" in data_dir:
            ...
            if node_type in YANG_OTHER_NE_TYPES:
                ...
                if isYANG:
                    ...
                    elif node_type in [ "CCPC" ] and depl_type == "NSS" and mim_ver not in [ "1-0-V1", "1-5-V1", "1-6-V1", "19-Q3-V1" ]:
                        attribute =  "/eric-pmbr-rop-file-store/"
                    else:
...
def get_mo_attribute_value(sim_name, node_name, mo_fdn, mo_attribute, node_type):
    ...
    # node types which support files with new MO element = CcpdService would be unique
    if "CcpdService" in mo_fdn:
        fetch_attribute="PD" 
    elif mo_attribute in ["fileLocation", "performanceDataPath", "file-location"]:
    ...
    if (not attribute.strip() or attribute.strip() == '""') and config.has_option(fetch_attribute,node_type.upper()):
...
def configFileReaderAndLogCreator(depl_type):
    ...
    configFileName = 'NSS.ini' if depl_type == 'NSS' else ('DO.ini' if depl_type == 'DO' else 'NRM.ini')
