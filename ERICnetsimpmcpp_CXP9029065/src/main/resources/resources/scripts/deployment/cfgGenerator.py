...
def upload_cfg(nssRelease="16.8", sim_data_list=[], edeStatsCheck="False", deplType="NSS", counterVolume="None", oss_enabled="False"):
    ...
    if deplType == "NSS":
        if oss_enabled == "True":
            ...
            template = "/netsim_users/auto_deploy/etc/netsim_cfg_template"
    elif "NRM" in deplType:
        template = "/netsim_users/auto_deploy/etc/netsim_cfg_template_" + deplType
    else:
        print 'ERROR : Invalid deployment type.'
    create_netsim_cfg(get_hostname(), nssRelease, ' '.join(sims), ' '.join(mmes), PM_file_paths, playback_sim_list.strip(), template, edeStatsCheck, counterVolume, oss_enabled)
...
def get_bsc_list():
    ...
    result = run_shell_command(bashCommand).split()
    if result:
        return result
    else:
        return None
