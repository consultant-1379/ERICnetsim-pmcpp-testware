...
def main(argv):
    if not argv:
        print " Usage:    /netsim/bin/updateConfiguration.py <--config update_FDN|cell_relation|update_filter|ALL>"
        sys.exit(1)
    try:
    ...
    for opt, arg in options:
        if opt == '--config':
            config = arg
    os.system("rm -rf /netsim/netsimdir/*/SimNetRevision/*")#Deleting TopologyData.txt and EUtranCellData.txt
    if config == "update_FDN":
        ...
        sys.exit(1)
    os.system("/usr/local/bin/python2.7 /netsim/bin/generateEutranDataFile.py")
