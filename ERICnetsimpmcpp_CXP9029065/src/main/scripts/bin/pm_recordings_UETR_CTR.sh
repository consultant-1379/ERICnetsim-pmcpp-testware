...
for SERVER in $SERVERS ; do
    ...
    if [ ${SERVER_HAS_WRAN} -eq 1 ] || [ ${SERVER_HAS_LTE} -eq 1 ] || [ ${SERVER_HAS_MME} -eq 1 ] || [ ${SERVER_HAS_FIVEG} -eq 1 ] ; then
        ...
        if [ ${SERVER_HAS_LTE} -eq 1 ] || [ ${SERVER_HAS_FIVEG} -eq 1 ]  ; then
            ...
            FIVEG_UETRACE_FILE_LIST="${NRAT_LTE_UETRACE_LIST}"
            ...
            if [[ ! -z "${FIVEG_UETRACE_FILE_LIST}" ]]; then
                LTE_UETRACE_FILE_LIST="${LTE_UETRACE_FILE_LIST} ${FIVEG_UETRACE_FILE_LIST}"
            fi
            ...
            if [[ ${TYPE} == "NRM6.2" ]] &&  [[ ${TYPE} == "NRM6.2.1" ]] && [[ ${SERVER_HAS_LTE} -eq 1 ]]; then
            ...
            if [ ${SERVER_HAS_FIVEG} -eq 1 ] ; then
                ...
                if [[ ${TYPE} == "NRM6.2" ]] || [[ ${TYPE} == "NRM6.2.1" ]] || [[ ${TYPE} == "NRM6.3" ]]; then
                ...
                elif [[ ${TYPE} == "NRM6.4" ]]; then
                    SPECIFIC_NRM_FIVEG_FILE_LIST=("celltrace_cucp_1.6MB.bin.gz" "celltrace_cuup_804KB.bin.gz" "celltrace_du_1.7MB.bin.gz" "celltrace_cucp_526KB.bin.gz" "celltrace_cuup_272KB.bin.gz" "celltrace_du_576KB.bin.gz")
                elif [[ ${TYPE} == "NSS" ]];then
                    SPECIFIC_NRM_FIVEG_FILE_LIST=("celltrace_cucp_94byte.bin.gz" "celltrace_cuup_91byte.bin.gz" "celltrace_du_91byte.bin.gz")
                fi
        ...
        if [ ${SERVER_HAS_WRAN} -eq 1 ] ; then
            ...
            for RECORDING_WORKLOAD in $RECORDING_WORKLOAD_LIST; do
                ...
                if [ ${SERVER_HAS_MME} -ne 1 ] && [[ "$ROP_PERIOD" == "1" ]];then
                    continue
                fi
                case "${ROP_PERIOD}" in
                ...
                echo "${DEFAULT_MINUTE_FIELD} * * * ${NETSIM_BIN_DIR}/wran_rec.sh -l DEFAULT -r ${ROP_PERIOD} >> ${NETSIM_LOG_DIR}/wran_rec.log 2>&1" >> /tmp/_new_crontab
                echo "${PEAK_MINUTE_FIELD} * * * ${NETSIM_BIN_DIR}/wran_rec.sh -l PEAK -r ${ROP_PERIOD} >> ${NETSIM_LOG_DIR}/wran_rec.log 2>&1" >> /tmp/_new_crontab
            done
        fi
            ...
            for RECORDING_WORKLOAD in $RECORDING_WORKLOAD_LIST; do
                ...
                if [ "${TYPE}" = "NSS" ] ; then
                    if [ ${NE_TYPES} = "ALL" ] ; then
                        echo "${MINUTE_FIELD} * * * * ${NETSIM_BIN_DIR}/lte_rec.sh -r ${ROP_PERIOD} >> ${NETSIM_LOG_DIR}/lte_rec_${ROP_PERIOD}min.log 2>&1" >> /tmp/_new_crontab
                    else
                   ...
                   if [[ "$ROP_PERIOD" == "15" ]];then
                        echo "${MINUTE_FIELD} * * * * /netsim_users/pms/bin/lte_rec.sh -r 15  >> /netsim_users/pms/logs/lte_rec_15min.log 2>&1" >> /tmp/_new_crontab
                   elif [[ "$ROP_PERIOD" == "1" ]];then
                        echo "* * * * * /netsim_users/pms/bin/lte_rec.sh -r 1 -f EBS:EBM >> /netsim_users/pms/logs/lte_rec_1min.log 2>&1" >> /tmp/_new_crontab
                   fi
                else
                    echo "${MINUTE_FIELD} * * * * /netsim_users/pms/bin/lte_rec.sh -r 15 -f CELLTRACE:UETRACE:CTUM >> /netsim_users/pms/logs/lte_rec_15min.log 2>&1" >> /tmp/_new_crontab
                    echo "* * * * * /netsim_users/pms/bin/lte_rec.sh -r 1 -f EBS:EBM >> /netsim_users/pms/logs/lte_rec_1min.log 2>&1" >> /tmp/_new_crontab
                fi
            done
        fi
