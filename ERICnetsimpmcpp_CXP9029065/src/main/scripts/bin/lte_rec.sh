...
generateCELLTRACE() {
    ...
    if [[ "${SIM_BEHAVIOUR}" == "MixedNRAT" ]]; then
        MSRBS_V2_NODE_LIST=`ls ${OUT_ROOT}/${SIM} | egrep -i 'dg2ERBS|MSRBS-V2|gNodeBRadio'`
    else
        MSRBS_V2_NODE_LIST=`ls ${OUT_ROOT}/${SIM} | egrep -i 'dg2ERBS|MSRBS-V2'`
    fi
            ...
            for NODE in ${ERBS_NODE_LIST} ; do
                ...
                if [[ ${SPECIFIC_CELLTRACE} == "LTE" ]]; then
                    if [[ ${NODE} == ${LTE_CELLTRACE_19MB_NODE} && ${REC_TYPE} = 1 ]]; then
                        ...
                        ln -s ${TMPFS_REC_TEMPLATES_DIR}/${LTE_CELLTRACE_19MB_FILE} ${NODE_PM_DATA}/${OUT_FILE_NAME}
                    else
...
generateERBSUETRACE() {
    ...
    if [[ ${TYPE} == "NSS" ]] || [[ ${TYPE} == "NRM6.2.1" ]]; then
        NRAT_NODE_LIST=($(ls ${OUT_ROOT}/${MY_SIM}/ | grep 'gNodeBRadio'))
    fi
        ...
        for LTE_UETRACE in ${LTE_UETRACE_LIST} ; do
            ...
            if [ "${UETRACE_SIM}" = "${MY_SIM}" ] ; then
                ...
                while [ ${CURR_REF} -le ${END_REF} ] ; do
                    ...
                    ln -s ${TMPFS_IN_FILE} ${OUT_ROOT}/${SIM}/${NODE}/c/pm_data/A${DATE}.${ROP_START_TIME}-${ROP_END_TIME}_uetrace_${REF}.bin.gz
        ...
        for LTE_UETRACE in ${MSRBS_V2_LTE_UETRACE_LIST} ; do
            ...
            if [ "${UETRACE_SIM}" = "${MY_SIM}" ] ; then
                ...
                while [ ${CURR_REF} -le ${END_REF} ] ; do
                    ...
                    ln -s  ${TMPFS_IN_FILE} ${OUT_ROOT}/${SIM}/${NODE}/${FILE_PATH}/A${DATE}.${ROP_START_TIME}-${ROP_END_TIME}_uetrace_${REF}.bin.gz
                ...
                for (( NE=${START_NE}; NE <= ${END_NE}; NE++ )); do
                    ...
                    if grep -q ${node} "/tmp/showstartednodes.txt"; then
                        for ue_pm_path in ${FILE_PATH}; do
                            node_dir_path="${OUT_ROOT}/${SIM}/${node}${ue_pm_path}"
                            CURR_REF=${START_REF}
        ...
        if [ $? -eq 0 ] ; then
            ...
            for NE in ${NE_LIST}; do
                ...
                if [ ! -z "${MME_CTUM_LIST}" ] ; then
                    ...
                    CTUM_OUTPUTDIR="${OUT_ROOT}/${SIM}/${NE}/tmp/OMS_LOGS/ctum/ready/"
                    CTUM_NODEDIR="${SIM_DIR}/${SIM}/${NE}/fs/tmp/OMS_LOGS/ctum/ready/"
                    #Create ctum dir structure if not present
                    if [[ ! -d ${CTUM_OUTPUTDIR} ]] || [[ ${EXEC_FROM_HC} == "YES" ]] || [[ ! $(findmnt "${CTUM_NODEDIR}") ]];then
                        if [[ ${ROP_PERIOD_MIN} == 15 ]] ; then
                            echo $CTUM_OUTPUTDIR
                            echo $CTUM_NODEDIR
...
generateMMEUETRACE() {
    ...
    for SIM in ${MME_SIM_LIST}  ; do
        ...
        if [ $? -eq 0 ] ; then
            ...
            for NE in $NE_LIST; do
                if [ ! -z "${MME_UETRACE_LIST}" ] ; then
                    UETRACE_OUTPUTDIR="${OUT_ROOT}/${SIM}/${NE}/tmp/OMS_LOGS/ue_trace/ready"
                    UETRACE_NODEDIR="${SIM_DIR}/${SIM}/${NE}/fs/tmp/OMS_LOGS/ue_trace/ready"
                    # Create uetrace dir structure if not present
                    if [[ ! -d ${UETRACE_OUTPUTDIR} ]] || [[ ${EXEC_FROM_HC} == "YES" ]] || [[ ! $(findmnt "${UETRACE_NODEDIR}") ]];then
                        if [[ ${ROP_PERIOD_MIN} == 15 ]] ; then
...
generateCelltraceForConfSims(){
    python ${CONF_CELLTRACE_SCRIPT} ${DATE} ${ROP_START_TIME} ${ROP_END_TIME}
}
