...
# Jira No       :  NSS-47794
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/17925405/
# Description   :  added support for vCU* and vDU in NRM
# Date          :  29/05/2024
...
PMS_TMPFS_GEN_NODES="ECEE EPG-OI vEPG-OI WMG-OI vWMG-OI gNodeBRadio SBG-IS FrontHaul-6020 FrontHaul-6000 FrontHaul-6080 vCU-UP vCU-CP vAFG ADP SCEF"
EPG_OI_TYPE_DST_NODES="EPG-OI vEPG-OI WMG-OI vWMG-OI ADP vDU vCU-UP vCU-CP O1"
EPG_OI_TYPE_DST_SWITCH_SUPPORTED="EPG-OI vEPG-OI"
# NSS and 15 min ROP only
NSS_ONLY_NE_TYPES="ECEE ADP vWMG-OI WMG-OI vAFG"
DO_SUPPORTED_NE_TYPES="vWMG-OI vEPG-OI vAFG VCUDB"
MD_1_SUPPORTED_NE_TYPES="SCEF vCU-UP vCU-CP SBG-IS HSS-FE"
# Sims that are supported by NSS and any other deployment other than NRM
NOT_SUPPORTED_IN_NRM="SCEF O1"
NSS_ONLY_NE_TYPES_EXCLUDING_1MIN="FrontHaul-6000"
...
EPG_OI_ROP_END_TIME_LOCAL=$(date -d @${end_epoch} +'%H%M')
...
if [[ ${ROP_LOCAL_OFFSET} != ${ROP_LOCAL_END_OFFSET} ]];then
   ...
   EPG_OI_UTC_END_TIME=$(date -u -d @${end_epoch} +'%H%M')
   ...
   if [[ ${OFFSET_TYPE} == "+" ]];then
       ...
       EPG_OI_ROP_END_TIME_LOCAL=$(date -u -d "${EPG_OI_UTC_END_TIME} +${OFFSET_HOUR} hour $OFFSET_MIN minutes " +"%H%M")
   else
       FH_ORAN_ROP_END_TIME_LOCAL=$(date -u -d "${FH_ORAN_UTC_END_TIME} -${OFFSET_HOUR} hour ${OFFSET_MIN} minutes " +"%H%M")
       EPG_OI_ROP_START_TIME_LOCAL=$(date -u -d "${EPG_OI_UTC_START_TIME} -${EPG_OI_OFFSET_HOUR} hour $EPG_OI_OFFSET_MIN minutes " +"%H%M")
       EPG_OI_ROP_END_TIME_LOCAL=$(date -u -d "${EPG_OI_UTC_END_TIME} -${OFFSET_HOUR} hour $OFFSET_MIN minutes " +"%H%M")
...
TWENTY_FOUR_HOUR_SIMS="FrontHaul-6020 FrontHaul-6000 FrontHaul-6080"
...
for SIM_NAME in ${PLAYBACK_SIM_LIST}; do
    ...
    if grep -q ${SIM_NAME} "/tmp/showstartednodes.txt"; then
        ...
        for NE in ${NE_TYPE_LIST}; do
            ...
            if [[ $NE == "CUDB" ]] || [[ $NE == "AFG" ]] || [[ $NE == "WMG-OI" ]] || [[ $NE == "EPG-OI" ]]; then
                echo ${SIM_NAME} | grep -i "V${NE}" > /dev/null
                if [[ $? -eq 0 ]]; then
                    continue
                fi
            fi
            ...
            if [[ "${SIM_NAME}" == *${NE}* ]]; then
                if [[ ${TYPE} == "NRM"* ]];then
                    echo ${NOT_SUPPORTED_IN_NRM} | grep -w ${NE/_/-} > /dev/null
                    if [[ $? -eq 0 ]];then
                        log "${NE} Not supported in NRM environment."
                        continue
                    fi
                fi
                if [[ ${TYPE} == "DO" ]];then
                    echo ${DO_SUPPORTED_NE_TYPES} | grep -i ${NE} > /dev/null
                    if [[ $? -ne 0 ]];then
                        continue
                    fi
                # stop file gen for unsupported nodes in MD_1
                elif [[ ${TYPE} == "MD_1" ]];then
                    echo ${MD_1_SUPPORTED_NE_TYPES} | grep -i ${NE} > /dev/null
                    if [[ $? -ne 0 ]];then
                        continue
                    fi
                elif [[ ${TYPE} == "NSS" && ${ROP_PERIOD_MIN} -ne 15 ]] || [[ ${TYPE} != "NSS" ]];then
                    echo ${NSS_ONLY_NE_TYPES} | grep -i ${NE} > /dev/null
                   #For file generation of node types in NSS_ONLY_NE_TYPES
                    if [[ $? -eq 0 ]];then
                        log "${NE} only supports 15min ROP in NSS environment."
                        continue
                    fi
                fi
                # Node Types that are not supported with 1min ROP in NSS environment and file generation in other environments."
                if [[ ${TYPE} == "NSS" && ${ROP_PERIOD_MIN} == 1 ]] || [[ ${TYPE} != "NSS" ]];then
                    echo ${NSS_ONLY_NE_TYPES_EXCLUDING_1MIN} | grep -i ${NE} > /dev/null
                    #For file generation of node types in NSS_ONLY_NE_TYPES_EXCLUDING_1MIN
                    if [[ $? -eq 0 ]];then
                        continue
                    fi
                fi
                # only 15 min supported for SCEF nodes and only in NSS and MD_1
                if [[ ${NE} == "SCEF" ]] && [[ ${ROP_PERIOD_MIN} != 15 ]] ; then
                    continue
                fi
                # only 15 min supported for SBG-IS nodes in MD_1
                if [[ ${NE} == "SBG-IS" ]] && [[ ${ROP_PERIOD_MIN} != 15 && ${TYPE} == "MD_1" ]] ; then
                    continue
                fi
                ...
                if [[ ${!stats} == "YES" ]] ; then
                    ...
                    if [[ ${NE} == *EPG_OI* ]] || [[ -z ${!append_path} ]];then
                        if [[ ${NE} == "CUDB" && ${TYPE} == "NRM"* ]] ; then
                            append_path="$NE"_STATS_APPEND_PATH_NRM
                        else
                            append_path="$NE"_STATS_APPEND_PATH
                        fi
                    fi

                    if [[ ${!stats_TZ} == "LOCAL" ]] || [[ ${NE} == "CUDB" && ${TYPE} == "NRM"* && ${!stats_TZ} == "UTC" ]] ; then
                        DATE_REF=${DATE_LOCAL}
                        if [[ ${NE} == "SBG_IS" ]] ; then
                            ...
                            if [[ ${NE} == *"FrontHaul"* ]] ; then
                                ...
                                if [[ $? -eq 0 ]];then
                                   echo ${EPG_OI_TYPE_DST_SWITCH_SUPPORTED} | grep -w ${NE/_/-} >> /dev/null
                                   if [[ $? -eq 0 ]] && [[ $ROP_PERIOD_MIN == 1440 || $ROP_PERIOD_MIN == 720 ]];then
                                       ROP_START_TIME_STATS=${ROP_START_TIME_LOCAL}${EPG_OI_ROP_START_OFFSET_LOCAL}
                                       ROP_END_TIME_STATS=${EPG_OI_ROP_END_TIME_LOCAL}${ROP_LOCAL_END_OFFSET}
                                   elif [[ $ROP_PERIOD_MIN == 1440 ]];then
                    ...
                    if [[ ${NE} == "SBG_IS" ]] ; then
                        /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${UNIQUE_ID}" &
                        ...
                        for FULL_JOB_NAME in ${!MEASUREMENT_JOB_NAME}; do
                            ...
                            if [[ ${NE} == *"EIR_FE"* ]]; then
                                if [[ ${TYPE} != "NSS" ]]; then
                                   /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}_${JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${FULL_JOB_NAME}" &
                                else
                                    if [[ ${JOB_NAME} == *"CpuStatsJob"* ]] || [[ ${JOB_NAME} == *"DiameterStatsJob"* ]] || [[ ${JOB_NAME} == *"DiskStatsJob"* ]]; then
                                       UTC_OFFSET='+0000'
                                       /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}${UTC_OFFSET}" "${ROP_END_TIME_STATS}${UTC_OFFSET}" "${LOG}_${JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${FULL_JOB_NAME}" &
                                    else
                                       /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}_${JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${FULL_JOB_NAME}"  &
                                    fi
                                fi 
                                ...
                                for VCUDB_JOB_NAME in $(echo $VCUDB_JOB_CONFIGURATION | sed "s/,/ /g"); do
                                    /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}_${VCUDB_JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${VCUDB_JOB_NAME}" ""
                                done 
                            else
                                /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}/${JOB_NAME}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}_${JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${FULL_JOB_NAME}" "${TSP_MPID_TIME_START_DATETIME}" &
                            fi
                        done

                    elif [[ ${NE} == *"FrontHaul"* ]] && [[ ${!stats_update_TZ} == "YES" ]]; then
                        /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${FILE_beginTime}" "${FILE_endTime}" "${dur}" &
                        ...
                        for _file_type in ${!file_types};do
                            ...
                            /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_path}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${_file_type}" 
                        done
                    elif [[ ${NE} == "vAFG" ]] ; then
                        /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}_${JOB_NAME}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "" "" 
                        ...
                        if [[ ${ROP_PERIOD_MIN} == 15 ]];then
                            ...
                            for info in ${!oru_info};do
                               ...
                               time_zone=$(echo ${info} | awk -F"|" '{print $3}')
                                ...
                                /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "${hardware_name}"
                            done
                        fi
                    elif [[ ${NE} == *WMG*OI* ]] || [[ ${NE} == *ECEE* ]] || [[ ${NE} == *EPG*OI* ]] || [[ ${NE} == *vCU*UP* ]] || [[ ${NE} == *vCU*CP* ]] || [[ ${NE} == *ADP* ]];then
                        /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!stats_file_format}" "${!stats_output_type}" "${stats_input_location}" "${DATE_REF}" "${ROP_START_TIME_STATS}" "${ROP_END_TIME_STATS}" "${LOG}" "stats" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" "" ""
                ...
                if [[ ${!events} == "YES" ]] ; then
                    ...
                    /${INSTALL_DIR}/startFileDistribution.sh "${PM_DIR}" "${!append_path}" "${SIM_NAME}" "${!events_file_format}" "${!events_output_type}" "${!events_input_location}" "${DATE_REF}" "${ROP_START_TIME_EVENTS}" "${ROP_END_TIME_EVENTS}" "${LOG}" "events" "${script}" "${ROP_PERIOD_MIN}" "${ROP_END_DATE}" "${EXEC_FROM_HC}" &
                fi
             break
            fi
        done
