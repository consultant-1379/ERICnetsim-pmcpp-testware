...
# Description   :  Adding support vCU* and vDU in NRM
# Date          :  29/05/2024
# Last Modified :  surendra.mattaparthi.ext@ericsson.com
#################################################### 
...
NETSIM_PRODUCT_DATA_TEMPLATE_DIR=/netsim_users/pms/product_data_templates/
...
GENSTATS_SANDBOX_DEFAULT_NRM_TEMPLATE_DIR=/netsim/genstats/sandbox_templates_default_NRM
GENSTATS_PRODUCT_DATA_TEMPLATE_DIR=/netsim/genstats/product_data_templates
TMPFS_PLAYBACKER_NODES="ECEE EPG-OI vEPG-OI WMG-OI vWMG-OI gNodeBRadio SBG-IS FrontHaul-6020 FrontHaul-6000 FrontHaul-6080 vCU-UP vCU-CP vAFG ADP SCEF"
TMPFS_SANDBOX_DIR=/pms_tmpfs/xml_step/sandbox_templates/
TMPFS_XML_TEMPLATES_DIR=/pms_tmpfs/xml_step/xml_templates/
TMPFS_PRODUCT_DATA_TEMPLATES_DIR=/pms_tmpfs/xml_step/product_data_templates/
SENT_LIST=""
...
for SERVER in ${SERVERS} ; do
    ...
    if [[ "${TYPE}" == "NRM"* ]]; then
        /bin/cp -r ${GENSTATS_SANDBOX_DEFAULT_NRM_TEMPLATE_DIR}/* ${NETSIM_SANDBOX_TEMPLATE_DIR}
    fi
    ...
    #### MAKE THE SAME FOR PCC/PCG
    log "INFO: Deleting old product_data_templates directory and creating new one"

    `rm -rf ${NETSIM_PRODUCT_DATA_TEMPLATE_DIR} > /dev/null 2>&1`

    `mkdir  ${NETSIM_PRODUCT_DATA_TEMPLATE_DIR}`

    log "INFO: Sending product data templates."
    /bin/cp  ${GENSTATS_PRODUCT_DATA_TEMPLATE_DIR}/* ${NETSIM_PRODUCT_DATA_TEMPLATE_DIR}
    
    #delete product data templates in /pms_tmpfs/xml_step/ location
    if [[ -d ${TMPFS_PRODUCT_DATA_TEMPLATES_DIR} ]];then
        log "INFO : Deleting product data templates in /pms_tmpfs"
        rm -rf ${TMPFS_PRODUCT_DATA_TEMPLATES_DIR}
    fi
    `mkdir ${TMPFS_PRODUCT_DATA_TEMPLATES_DIR}`
    /bin/cp  ${NETSIM_PRODUCT_DATA_TEMPLATE_DIR}/* ${TMPFS_PRODUCT_DATA_TEMPLATES_DIR}

    
    if [[ "${TYPE}" == "DO" ]]; then
        mkdir -p ${TMPFS_XML_TEMPLATES_DIR}/15/PCC/
        ###mkdir -p ${TMPFS_XML_TEMPLATES_DIR}/15/PCC_AMF/
        mkdir -p ${TMPFS_XML_TEMPLATES_DIR}/15/PCG/
    fi
