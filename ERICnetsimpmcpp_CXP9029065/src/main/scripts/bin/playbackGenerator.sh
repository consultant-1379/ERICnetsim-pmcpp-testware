...
# Jira No       :  NSS-36326
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/12366389
# Description   :  Adding SBG-IS support in MD_1
# Date          :  29/04/2022
...
if [[ $NE_TYPE == *"FrontHaul"* ]]; then
    ...
    # handling symbolic link for all pm files in SBG-IS except the first one due to the file update
    if [[ ${DEPL_TYPE} == "MD_1" ]] && [[ $NE_TYPE == *"SBG-IS"* ]] ; then
        update_files=0
    fi
    # using hardlink as updating single template file each rop. So no need to keep a copy of each updated file and then softlinking. With hardlink it is possible to have a single file being changed each ROP and without the links being broken. 
    ln -f $source $target
else
    # softlink
    ln -sf $source $target
