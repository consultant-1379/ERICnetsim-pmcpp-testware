...
if [[ $NE_TYPE == *"SBG-IS"* ]]; then
    UNIQUE_ID=${16}
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "$UNIQUE_ID" &

elif [[ $NE_TYPE == *"HSS-FE-TSP"* ]] || [[ $NE_TYPE == *"MTAS-TSP"* ]] || [[ $NE_TYPE == *"CSCF-TSP"* ]] || [[ $NE_TYPE == *"SAPC-TSP"* ]] ; then
    MEASUREMENT_JOB_NAME=${16}
    TSP_MPID_TIME_START_DATETIME=${17}
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "${MEASUREMENT_JOB_NAME}" "$TSP_MPID_TIME_START_DATETIME" &

elif [[ $NE_TYPE == *"FrontHaul"* ]]; then
    FILE_beginTime=${16}
    FILE_endTime=${17}
    dur=${18}
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "$FILE_beginTime" "$FILE_endTime" "$dur" &

elif [[ $NE_TYPE == *"EIR-FE"* ]] || [[ $NE_TYPE == *"VCUDB"* ]]; then
    MEASUREMENT_JOB_NAME=${16}
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "${MEASUREMENT_JOB_NAME}" &

elif [[ $NE_TYPE == *"SCEF"* ]]; then
        if [[ ${FILE_TYPE} == B ]];then
            /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$mappedFileListScefB" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "${FILE_TYPE}" 
        else
            if [[ ${DEPL_TYPE} != "MD_1" ]]; then
               INPUT_LOCATION=${INPUT_LOCATION}/.tmpScefFiles/ 
            fi
            /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$mappedFileListScefA" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "${FILE_TYPE}"
        fi
elif [[ $NE_TYPE == *"gNodeBRadio"* ]]; then
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" "${HARDWARE_NAME}" 
else
    /${INSTALL}/$SCRIPT "$PM_DIR" "$APPEND_PATH" "$NE_TYPE" "$FILE_FORMAT" "$OUTPUT_TYPE" "$INPUT_LOCATION" "$DATE" "$ROP_START_TIME" "$ROP_END_TIME" "$INSTALL" "$LOG" "$completeMappedFileList" "$PROCESS_TYPE" "$ROP_PERIOD" "$ROP_END_DATE" "${EXEC_FROM_HC}" &
fi
