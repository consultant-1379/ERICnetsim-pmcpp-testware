...
HOST=`hostname`
HOST_NAME=`echo $HOST`
FLEX_SIM_LIST_NSS=($(cat /netsim/genstats/tmp/sim_info.txt | egrep -i "SGSN|R6274|R6675|R6672|R6673|R6676|R6678|R6371|R6471-1|R6471-2|R6273|CSCF|SBG|MTAS|VBGF|MRF|WCG|DSC|VSAPC" | egrep -v "TSP|SBG-IS" | awk -F":" '{ print $2 }' | sort -u))
FLEX_SIM_LIST_NRM=($(cat /netsim/genstats/tmp/sim_info.txt | egrep -i "MTAS|DSC" | egrep -v "TSP" | awk -F":" '{ print $2 }' | sort -u))
FLEX_SIM_LIST_MD_1=($(cat /netsim/genstats/tmp/sim_info.txt | egrep -i "R6675|R6672" | awk -F":" '{ print $2 }' | sort -u))
ML_OUTDOOR_SIM_LIST=($(ls ${NETSIM_DBDIR} | egrep "ML.*6352|ML.*6351|MLPT2020|FrontHaul-6392|Switch6391"))
ESC_SIM_LIST=($(ls ${NETSIM_DBDIR} | egrep "ESC"))
LOCAL_OFFSET=`date +%z`
...
if [[ "${TYPE}" = "NRM"* ]]; then	
    ...
    # get an array of only R6672 and R6675 for sim_data.txt file
    IFS=$'\n'
    ROUTER_NODE_TYPE_R6672_R6675_ARRAY=($(cat $SIM_DATA_FILE | egrep "R6672|R6675"))
    set IFS

    for line in ${ROUTER_NODE_TYPE_R6672_R6675_ARRAY[*]}; do
        if [[ ${line}  == *"Transport"* ]] ; then
           ROUTER_NODE_TYPE_R6672_R6675=($(echo "$line" | egrep -i 'R6672|R6675' | awk -F" " '{print $6}'))
           ROUNTER_NODE_NAME=($(echo "$line" | awk -F" " '{print $4}'))
           ROUTER_SIM_NAME=($(echo "$line" | awk -F" " '{print $2}'))
           break
        fi
...
echo $PLAYBACK_SIM_LIST | egrep "FrontHaul-6000" > /dev/null
if [[ $? == 0 ]];then
    PLAYBACK_FLEX_SIM_LIST+='FrontHaul-6000'
fi
...
for SERVER in $SERVERS ; do
    ...
    if [[ ${#ESC_SIM_LIST[@]} -ne 0 ]];then
        python /netsim_users/auto_deploy/bin/counterUpdater.py --createJson
    fi

    echo "00 23 * * * /netsim_users/pms/bin/logs_rotation.sh" >> /tmp/_new_crontab
    ...
    if [[ "${TYPE}" = "NSS" ]]; then
        ...
        if ! [[ -z ${FLEX_SIM_LIST_MD_1} ]]; then
            NODE_LIST=${FLEX_SIM_LIST_MD_1[@]}
            for node in ${NODE_LIST[@]};do
                /netsim_users/pms/bin/configFlexRop.sh -l ${node} -r 1440 -c 'ADD'
            done 
        fi
        if [[ ${ROUNTER_NODE_NAME} ]] && [[ ${ROUTER_NODE_TYPE_R6672_R6675} ]]; then
            echo "* * * * *  /netsim_users/pms/bin/genStats -r 1 -l '${ROUTER_NODE_TYPE_R6672_R6675}'  >> ${NETSIM_LOG_DIR[0]}/genStats_1min.log 2>&1" >> /tmp/_new_crontab
            python /netsim_users/auto_deploy/bin/generateSelectiveNeConf.py --sim ${ROUTER_SIM_NAME} --rop 1 --count 40
        fi
    fi
