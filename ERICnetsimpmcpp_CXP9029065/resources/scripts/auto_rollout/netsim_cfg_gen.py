...
def create_netsim_cfg(server_name, nssRelease, simulations, mmes, pm_file_paths, playback_sim_list, mytemplate, edeStatsCheck):
    ...
    with open(server_name, 'w+') as f:
        f.write(mytemplate.render(release=nssRelease, servers=server_name, server=server_name.replace(
            "-", "_"), simulation_list=simulations, mme_list=mmes, pm_file_locations=pm_file_paths, playback_sim_list=playback_sim_list, edeStatsCheck=edeStatsCheck ))
