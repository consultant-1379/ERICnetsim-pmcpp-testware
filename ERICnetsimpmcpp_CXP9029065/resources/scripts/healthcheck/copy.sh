...
# Jira No       :  NSS-13017
# Gerrit Link   :  https://gerrit.ericsson.se/#/c/2455580/
...
if [[ ! -z ${PLAYBACK_SIM_LIST} ]];then
    SIM_LIST=${SIM_LIST}" "${PLAYBACK_SIM_LIST}
    python ${BIN_DIR}/genstats_checking.py -l ${SIM_LIST} `eval echo '$'${HOSTNAME/-/_}_mme_list` -ul ${LTE_UETRACE_LIST} ${MSRBS_V1_LTE_UETRACE_LIST} ${MSRBS_V2_LTE_UETRACE_LIST} -b ${SET_BANDWIDTH_LIMITING} -recwl ${RECORDING_WORKLOAD_LIST} -statswl ${STATS_WORKLOAD_LIST} -gpehwl ${GPEH_WORKLOAD_LIST} -rbsgpehwl ${GPEH_RBS_WORKLOAD} -gpehmpcells ${GPEH_MP_CONFIG_LIST} -playbacksimlist ${PLAYBACK_SIM_LIST}
else
    python ${BIN_DIR}/genstats_checking.py -l ${SIM_LIST} `eval echo '$'${HOSTNAME/-/_}_mme_list` -ul ${LTE_UETRACE_LIST} ${MSRBS_V1_LTE_UETRACE_LIST} ${MSRBS_V2_LTE_UETRACE_LIST} -b ${SET_BANDWIDTH_LIMITING} -recwl ${RECORDING_WORKLOAD_LIST} -statswl ${STATS_WORKLOAD_LIST} -gpehwl ${GPEH_WORKLOAD_LIST} -rbsgpehwl ${GPEH_RBS_WORKLOAD} -gpehmpcells ${GPEH_MP_CONFIG_LIST}
fi
