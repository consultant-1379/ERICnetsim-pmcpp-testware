...
import com.ericsson.cifwk.netsim.ScannerTimeSyncOperator;
 ...
 */
public class ScannerTimeSyncTest extends TorTestCaseHelper implements TestCase {

private static final Logger logger = LoggerFactory.getLogger(ScannerTimeSyncTest.class);
	
    @Inject
    OperatorRegistry<ScannerTimeSyncOperator> operatorRegistry;

    @BeforeSuite
    public void init() {
        assertTrue(new ScannerTimeSyncOperator().initialise());
    }
     ...
     */
    @TestId(id = "ScannerTimeSyncTest", title = "Verify Scanner and time sync steps of GenStats")
    @Test(groups = { "Acceptance" })
    @DataDriven(name = "ScannerTimeSyncTest")
    @Context(context = { Context.CLI })
    public void verifyScannerTimeSync(@Input("command") final String command, @Output("expectedExitCode") final int expectedExitCode) {
		final UnitTestOperatorInterface seOperator = operatorRegistry.provide(ScannerTimeSyncOperator.class);
