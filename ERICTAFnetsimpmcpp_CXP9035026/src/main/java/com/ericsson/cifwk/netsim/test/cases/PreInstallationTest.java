...
import com.ericsson.cifwk.netsim.PreInstallationOperator;
 ...
 */
public class PreInstallationTest extends TorTestCaseHelper implements TestCase {

private static final Logger logger = LoggerFactory.getLogger(PreInstallationTest.class);
	
    @Inject
    OperatorRegistry<PreInstallationOperator> operatorRegistry;

    @BeforeSuite
    public void initialise() {
         assertTrue(new PreInstallationOperator().initialise());
    }
     ...
     */
    @TestId(id = "NETSUP-2838", title = "Verify pre installation steps of GenStats")
    @Test(groups = { "Acceptance" })
    @DataDriven(name = "PreInstallationTest")
    ...
    public void verifyPreInstallation(@Input("command") final String command, @Output("expectedExitCode") final int expectedExitCode) {
        final UnitTestOperatorInterface seOperator = operatorRegistry.provide(PreInstallationOperator.class);
