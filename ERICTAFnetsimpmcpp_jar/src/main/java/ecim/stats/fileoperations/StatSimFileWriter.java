...
import org._3gpp.ftp.specs.archive._32_series._32.MeasCollecFile;
...
import org._3gpp.ftp.specs.archive._32_series._32.ObjectFactory;
...
public class StatSimFileWriter {
  ...
  */
	...
	public void writeTemplateFile(
			...
			final String fileName, Map<String, MocCfgDataHolder> mocCfgData,
			Map<String, String> mocRealtions, final String nodeType,
			Map<String, String> mocNameMapToPmGroupId, String node_type,
			Hierarchy mimRelation) {
		/*
		 * Create a object factory which is responsible for creation of object
		 * which represents the xml structure.
		 */
		...
		for (String moClassName : getSortedList(mocClassToMeasTypes.keySet())) {
			...
			if (mocCfgData.containsKey(moClassName)) {
				MeasInfo measInfo = factory
						.createMeasCollecFileMeasDataMeasInfo();
				...
				if ((node_type.equalsIgnoreCase("vpp")) || (node_type.equalsIgnoreCase("vrm")) || (node_type.equalsIgnoreCase("vrsm")) || (node_type.equalsIgnoreCase("vtif"))) {
				...
				RepPeriod ropP = factory
						.createMeasCollecFileMeasDataMeasInfoRepPeriod();
				...
				List<String> parentDnList = getParentDnList(moClassName,
						mimRelation, mocCfgData.get(moClassName)
						.getNumOfInstance());
				if(parentDnList.isEmpty()){
					for(int instance=0; instance < mocCfgData.get(moClassName)
							.getNumOfInstance(); instance++ ){
						parentDnList.add(getMeasObjLdn(moClassName, mocRealtions));
					}
				}
				for (String moID : parentDnList) {

					MeasValue measValue = factory
							.createMeasCollecFileMeasDataMeasInfoMeasValue();
					String parentDn = moID;
					measValue.setMeasObjLdn(parentDn);
					index = 1;
					for (MeasurementType meas : measList) {
						R r = factory
						...
						if (meas.getMeasValueType() == MeasValueType.MULTI_VALUE) {
							for (int i = 0; i < meas.getMultiplicity() - 1; i++) {
								measVal.append(
										Long.toString(Math.abs(new Random()
												.nextLong() % 100))).append(
										EcimStatsConstant.MEAS_VAL_SEP);
							}
						}
     ...
     * @return measObjLdn ID
     */
	 List<String> getParentDnList(String moClass, Hierarchy mim, long instance) {
	       List<String> instanceList = new LinkedList<String>();
	        if(moClass != null){
	            List<String> parentList = null;
	            parentList = mim.returnECIMParent(moClass);
	            if(null != parentList){
	            if(parentList.contains("ManagedElement")){
	                parentList.clear();
	            ...
	            if(parentList!=null)
	            {
	            	for(String parentMoClass : parentList){
	            		List<String> parentInst = getParentDnList(parentMoClass,mim,1);
	            		long instCount = instance;
	            		for(Iterator<String> parentItr = parentInst.iterator(); parentItr.hasNext();){
	            			String currentGroup = parentItr.next();
	            			String instBase = currentGroup + "," + moClass + "=";
	                        for(int i = 1; i <= instCount; i++){
	                        	if (EcimStatsMain.isTddFlag_()){
	                        		instanceList.add((instBase + String.valueOf(i)).replace(",EUtranCellFDD=", ",EUtranCellTDD="));
	                        	}else{
